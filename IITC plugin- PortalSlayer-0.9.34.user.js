...
// @version        0.9.34
// @description    [0.9.34] Faction-based Portal Counter
// @id             portal-slayer
...
function wrapper(plugin_info) {
  ...
  const DEFAULT_CONFIG = {
    // レベル設定
    0: { active: false, color: '#CCCCCC' }, // Neutral (L0)
  ...
  const DEFAULT_OPTS = {
    ...
    autoUpdateMarkers: false, // [16] Auto Update Markers
    tapMode: 'toggle', // [17] Tap Mode Radio Buttons
    markVisibleMode: 'toggle', // [18] Bulk Add Mode
  ...
  S.loadSettings = function() {
    try {
      ...
      if (c) {
        ...
        for (let i = 0; i <= 8; i++) { // [4] Include L0
      ...
      if (o) {
        ...
        // Ensure tapMode
        if (!S.options.tapMode) S.options.tapMode = 'toggle';
        if (!S.options.markVisibleMode) S.options.markVisibleMode = 'toggle';
      }

      // Restore Delete Mode CSS state if necessary
      if (S.options.tapMode === 'delete') {
  ...
  S.drawMarker = function(guid, latlng, color, title) {
    ...
    marker.on('click', function(e) {
      if (S.options.tapMode === 'delete' || S.options.tapMode === 'toggle') {
        L.DomEvent.stop(e);
        S.removePortal(guid);
      }
    });
  ...
  S.addPortal = function(guid, latlng, level, color, title, skipListUpdate = false) {
  ...
  // --- Function 00075 Auto Update Logic
  // ============================================================
  // [16] ポータル情報更新時の自動マーカー処理
  // ポータル情報が読まれた時、▼マーカーが、指定したレベルと合致しない場合の挙動を指定する。
  // ============================================================
  S.handlePortalUpdate = function(portal) {
    if (!S.options.autoUpdateMarkers) return;
    if (!portal || !portal.options) return;

    const guid = portal.options.guid;

    // Check if tracked in current area
    if (!S.data.portals || !S.data.portals[guid]) return;
    if (S.data.portals[guid].areaIndex !== S.data.currentArea) return;

    const detail = portal.options.data;
    // Basic tile data usually has level. If missing, skip.
    if (!detail || typeof detail.level === 'undefined') return;

    const newLvl = Math.floor(detail.level);

    // [16] Fix: Absolutely remove marker if portal is Neutral (Lv0)
    if (newLvl === 0) {
        S.removePortal(guid);
        return;
    }

    const oldLvl = S.data.portals[guid].level;

    if (newLvl !== oldLvl) {
       // Check if new level is active
       // 例1: マーカーを付けたポータルが、レベルアップしてる場合、マーカーの色を変更する
       // 例2: ポータルが攻撃によりレベルを下げたとき... 指定のポータルレベルのマーカーに状態を変更する
       if (S.config[newLvl] && S.config[newLvl].active) {
           // Update
           S.data.portals[guid].level = newLvl;
           S.data.portals[guid].color = S.config[newLvl].color;
           // Title update if available
           if (detail.title) S.data.portals[guid].title = detail.title;

           S.saveData();
           // Redraw
           S.drawMarker(guid, {lat: S.data.portals[guid].lat, lng: S.data.portals[guid].lng}, S.data.portals[guid].color, S.data.portals[guid].title);
       } else {
           // 例2: チェック無しレベルに該当する場合、マーカーを削除する
           // Remove because it's no longer a target level
           S.removePortal(guid);
       }
    }
  };

  S.onPortalAdded = function(data) {
    S.handlePortalUpdate(data.portal);
  };

  S.onPortalDetailLoaded = function(data) {
    if (data.success && window.portals[data.guid]) {
        S.handlePortalUpdate(window.portals[data.guid]);
    }
  };

  S.onMapDataRefreshEnd = function() {
    if (!S.options.autoUpdateMarkers) return;
    if (!S.data.portals) return;
    const guids = Object.keys(S.data.portals);
    for (let i = 0; i < guids.length; i++) {
        ...
        if (window.portals[guid]) {
            S.handlePortalUpdate(window.portals[guid]);
        }
    }
  };
  ...
  S.onPortalSelected = function(data) {
    ...
    if (S.options.tapMode === 'none') return;
    ...
    // [4] Check config including Level 0
    if (level >= 0 && S.config[level] && S.config[level].active) {
      const existing = S.data.portals ? S.data.portals[guid] : null;

      if (S.options.tapMode === 'toggle') {
         // Toggle Mode ON: Add if missing, Remove if present
         if (existing && existing.areaIndex === S.data.currentArea) {
             S.removePortal(guid);
         } else {
             S.addPortal(guid, p.getLatLng(), level, S.config[level].color, title);
         }
      } else if (S.options.tapMode === 'add') {
         // Add only (Normal behavior)
         if (!existing || existing.areaIndex !== S.data.currentArea) {
             S.addPortal(guid, p.getLatLng(), level, S.config[level].color, title);
         }
      }
  ...
  S.openSettings = function() {
    ...
    // [4] Color Preset Options
    let colorPresetOptions = '<option value="">Preset...</option>';
    for (const colorName in BASIC_COLORS) {
        const colorVal = BASIC_COLORS[colorName];
        colorPresetOptions += `<option value="${colorVal}" style="background:${colorVal}; color:${(colorName==='White'||colorName==='Yellow'||colorName==='Lime'||colorName==='Aqua'||colorName==='Cyan'||colorName==='Silver') ? '#000' : '#FFF'};">${colorName}</option>`;
    }
      ...
      <div class="portal-slayer-settings">
        <div class="ps-header">
           ...
           <div style="margin-top:8px; border-top:1px solid #444; padding-top:4px;">
             <div style="font-weight:bold; color:#ddd;">ラベル表示・更新設定:</div>
             ...
             <div style="margin-top:4px;"><label><input type="checkbox" id="ps-auto-update" ${S.options.autoUpdateMarkers ? 'checked' : ''}> ポータルレベルを自動追従</label></div>
           </div>
          ...
          ${[0,1,2,3,4,5,6,7,8].map(lvl => {
            ...
            return `
              <tr>
                ...
                <td>
                    <input type="color" class="ps-lvl-color" data-lvl="${lvl}" value="${c.color}">
                    <select class="ps-lvl-preset" data-lvl="${lvl}" style="width:70px; margin-left:4px;">${colorPresetOptions}</select>
                </td>
              </tr>
            `;
          }).join('')}
        ...
        <div class="ps-controls">
          <div style="margin-bottom:8px; border-top:1px solid #444; padding-top:4px;">
             ...
             <div style="display:flex; flex-direction:column; gap:4px;">
               ...
               <label><input type="radio" name="ps-tap-mode" value="toggle" ${S.options.tapMode === 'toggle' ? 'checked' : ''}> トグルモード</label>
               <label><input type="radio" name="ps-tap-mode" value="delete" ${S.options.tapMode === 'delete' ? 'checked' : ''}> マーカー削除のみ</label>
             </div>
          </div>
          <div style="margin-bottom:8px; border-top:1px solid #444; padding-top:4px;">
             <div style="font-weight:bold; color:#ddd; margin-bottom:4px;">一括操作 [18]:</div>
             <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
               <label style="flex:1;">動作モード:</label>
               <select id="ps-mark-visible-mode" style="flex:1;">
                 <option value="add" ${S.options.markVisibleMode === 'add' ? 'selected' : ''}>マーカー付与のみ</option>
                 <option value="toggle" ${S.options.markVisibleMode === 'toggle' ? 'selected' : ''}>トグルモード</option>
                 <option value="delete" ${S.options.markVisibleMode === 'delete' ? 'selected' : ''}>マーカー削除のみ</option>
               </select>
             </div>
             <button id="ps-btn-mark-visible" style="width:100%;">視界内の対象ポータルに処理を実行</button>
          </div>
    ...
    $('#ps-auto-update').on('change', function() { S.options.autoUpdateMarkers = this.checked; S.saveSettings(); });
    ...
    // [4] Color Preset Handler
    $('.ps-lvl-preset').on('change', function() {
        const val = this.value;
        if(val) {
           const lvl = $(this).data('lvl');
           $(`.ps-lvl-color[data-lvl="${lvl}"]`).val(val).trigger('change');
        }
    });

    // [17] Tap Mode Radio Handler
    $('input[name="ps-tap-mode"]').on('change', function() {
        S.options.tapMode = this.value;
        S.saveSettings();
    ...
    $('#ps-mark-visible-mode').on('change', function() {
        S.options.markVisibleMode = this.value;
        S.saveSettings();
    });

    $('#ps-btn-mark-visible').on('click', function() {
        if (!window.map || !window.portals) return;

        const bounds = window.map.getBounds();
        const portalsArr = Object.values(window.portals);
        let processedCount = 0;
        const mode = S.options.markVisibleMode || 'toggle';

        portalsArr.forEach(p => {
            ...
            const title = p.options.data.title;

            if (level >= 0 && S.config[level] && S.config[level].active) {
                const existing = S.data.portals ? S.data.portals[guid] : null;
                const isInCurrentArea = existing && existing.areaIndex === S.data.currentArea;
                ...
                if (mode === 'toggle') {
                    if (isInCurrentArea) {
                        ...
                        S.addPortal(guid, latlng, level, S.config[level].color, title, true);
                        processedCount++;
                    }
                } else if (mode === 'add') {
                    if (!isInCurrentArea) {
        ...
        if (processedCount > 0) {
            ...
            if ($('#portal-slayer-list-body').length > 0) {
                $('#portal-slayer-list-dialog').dialog('close');
                S.openListView();
            }
        }
        alert(`視界内の対象ポータル ${processedCount} 件を処理しました。`);
    });
  ...
  S.setupCSS = function() {
    if ($('#portal-slayer-css').length === 0) {
      $('<style>').prop('id', 'portal-slayer-css').prop('type', 'text/css').html(`
        ...
        .ps-controls button:disabled { opacity: 0.5; cursor: not-allowed; }
    ...
    try {
      ...
      const initMap = setInterval(function() {
        if (window.map && window.L) {
          ...
          // [16] Auto Update Hook
          window.removeHook('portalAdded', S.onPortalAdded);
          window.addHook('portalAdded', S.onPortalAdded);

          window.removeHook('portalDetailLoaded', S.onPortalDetailLoaded);
          window.addHook('portalDetailLoaded', S.onPortalDetailLoaded);

          window.removeHook('mapDataRefreshEnd', S.onMapDataRefreshEnd);
          window.addHook('mapDataRefreshEnd', S.onMapDataRefreshEnd);
...
(function() {
  var info = { "script": { "name": "IITC plugin: PortalSlayer", "version": "0.9.34", "description": "[0.9.34] Faction-based Portal Counter" } };
