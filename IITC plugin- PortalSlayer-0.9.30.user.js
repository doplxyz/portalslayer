...
// @version        0.9.30
// @description    [0.9.30] Faction-based Portal Counter
// @id             portal-slayer
...
function wrapper(plugin_info) {
  ...
  const DEFAULT_CONFIG = {
    // レベル設定 (0-8)
    0: { active: false, color: '#CCCCCC' }, // [4] Neutral / L0
  ...
  S.isToggleMode = true; // [15] Toggle Mode (Default ON)
  S.guidToLayer = {};
  ...
  S.loadSettings = function() {
    try {
      ...
      if (c) {
        ...
        // Deep merge level settings to ensure new defaults (0-8)
        for (let i = 0; i <= 8; i++) {
  ...
  S.drawMarker = function(guid, latlng, color, title) {
    ...
    marker.on('click', function(e) {
      if (S.isDeleteMode) {
        L.DomEvent.stop(e);
        S.removePortal(guid);
      } else if (S.isToggleMode) {
        // [15] Toggle Mode: Marker click deletes it
        L.DomEvent.stop(e);
        S.removePortal(guid);
      }
    });
  ...
  S.handlePortalUpdate = function(portal) {
    ...
    if (newLvl !== oldLvl) {
       ...
       if (S.config[newLvl] && S.config[newLvl].active) {
           ...
           S.saveData();
           // Redraw marker with new color/properties
           ...
           // 例2、ポータルが攻撃によりレベルを下げたとき、ポータルレベルは下がっているため指定のポータルレベルのマーカーに状態を変更する。
           // チェック無しレベルに該当する場合、マーカーを削除する。
           S.removePortal(guid);
       }
    }
  };
  ...
  S.onPortalSelected = function(data) {
    ...
    // Allow TEAM_NONE (Neutral/L0) to proceed if configured.
    ...
    if (level >= 0 && S.config[level] && S.config[level].active) {
      ...
      if (S.isToggleMode) {
         ...
         // Toggle Mode OFF: Add only (Normal behavior)
         if (!existing || existing.areaIndex !== S.data.currentArea) {
             S.addPortal(guid, p.getLatLng(), level, S.config[level].color, title);
         }
      }
  ...
  S.openSettings = function() {
    ...
    // Color Presets HTML Helper
    const generateColorPresetSelect = (lvl, currentColor) => {
       let opts = '<option value="">Preset</option>';
       for (const colorName in BASIC_COLORS) {
           const colorVal = BASIC_COLORS[colorName];
           opts += `<option value="${colorVal}" style="background-color:${colorVal}; color:${(colorName==='White'||colorName==='Yellow'||colorName==='Lime'||colorName==='Aqua'||colorName==='Silver')?'black':'white'}">${colorName}</option>`;
       }
       return `<select class="ps-color-preset" data-lvl="${lvl}" style="width:60px; margin-right:4px;">${opts}</select>`;
    };
      ...
      <div class="portal-slayer-settings">
        <div class="ps-header">
           ...
           <div style="margin-top:8px; border-top:1px solid #444; padding-top:4px;">
             ...
             <div style="margin-top:4px;"><label><input type="checkbox" id="ps-auto-update" ${S.options.autoUpdateMarkers ? 'checked' : ''}> ポータルレベルを自動追従 [16]</label></div>
           </div>
            ...
            return `
              <tr>
                ...
                <td style="display:flex; align-items:center; justify-content:center;">
                  ${generateColorPresetSelect(lvl, c.color)}
        ...
        <div class="ps-controls">
          <div style="display:flex; gap:4px; margin-bottom:4px;">
             <button id="ps-btn-toggle-mode" class="${S.isToggleMode ? 'active' : ''}" style="flex:1;">${S.isToggleMode ? 'トグルモード中 (ON)' : 'トグルモード OFF'}</button>
             <button id="ps-btn-delete-mode" class="${S.isDeleteMode ? 'active' : ''}" style="flex:1;" ${S.isToggleMode ? 'disabled' : ''}>${S.isDeleteMode ? '削除モード中 (ON)' : '削除モード OFF'}</button>
          </div>
    ...
    // [4] Preset Color Change Handler
    $('.ps-color-preset').on('change', function() {
       const color = this.value;
       if(color) {
           const lvl = $(this).data('lvl');
           // Update the color input and trigger change to save settings
           $(`.ps-lvl-color[data-lvl="${lvl}"]`).val(color).trigger('change');
       }
    });

    // [15] Toggle Mode Handler
    $('#ps-btn-toggle-mode').on('click', function() {
        S.isToggleMode = !S.isToggleMode;

        // Update Toggle Button UI
        $(this).text(S.isToggleMode ? 'トグルモード中 (ON)' : 'トグルモード OFF');
        $(this).toggleClass('active', S.isToggleMode);

        // Update Delete Mode Button UI/State
        const delBtn = $('#ps-btn-delete-mode');
        if (S.isToggleMode) {
            S.isDeleteMode = false;
            delBtn.text('削除モード OFF').removeClass('active').prop('disabled', true);
    ...
    $('#ps-btn-delete-mode').on('click', function() {
      if (S.isToggleMode) return; // Prevention
      S.toggleDeleteMode();
      $(this).text(S.isDeleteMode ? '削除モード中 (ON)' : '削除モード OFF');
...
(function() {
  var info = { "script": { "name": "IITC plugin: PortalSlayer", "version": "0.9.30", "description": "[0.9.30] Faction-based Portal Counter" } };
