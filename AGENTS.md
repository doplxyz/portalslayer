...
[15] ▼マーカーをタップで追加、再タップで削除出来るよう、トグル機能が必要。機能をON/OFFで切り替えれるようにする。デフォルトはOFF。削除モードとバッティングするため、トグル機能がONになったら、削除モードは強制的にオフにし、選択出来ないようにする。トグルモードがOFFの時は、通常通り▼マーカーだけ付けれる状態にする。
[16] ポータル情報が読まれた時、▼マーカーが、指定したレベルと合致しない場合の挙動を指定する。例えばポータルレベルに変動があった時、マーカーの色を現時点のマーカーの色に変更する。
例1、p7色でマーカーを付けたポータルが、p8にレベルアップしてる場合、マーカーの色を変更する。
例2、p8だけチェックしているプレイヤーが、攻撃によりポータルのレベルを下げたとき、ポータルレベルは下がっているため指定のポータルレベルのマーカーに状態を変更する。チェック無しレベルに該当する場合、マーカーを削除する。この機能は、ポータルレベル変動に応じてマーカーを変動させる旨のチェックボックスにてON/OFF可能にする。
