...
[4] ユーザーが指定したレベル（L0～L8）のポータルに対し、指定した色でマーカーを描画する。これらは視認性を高めつつ、マップ操作を阻害しない描画レイヤーで管理されなければならない。プリセットカラーはRGB基本16色を用意し選択出来るようにする。
...
[14] 主要コードの塊は 00000 , 00010 , 00020 , のように管理が必要だ。差分パッチにより新規ブロックが必要な場合は 00011のようにユーザが手で小刻みに変更を入れる。
...
[15] ▼マーカーをポータルタップで追加、再タップで削除出来るよう、トグル機能が必要。▼マーカーに触っても削除できるようにする。ラジオボタンで▼マーカー付与モードを切り替えられるため動作バッティングを考慮不要。
[16] ポータル情報が読まれた時、▼マーカーが、指定したレベルと合致しない場合の挙動を指定する。例えばポータルレベルに変動があった時、マーカーの色を現時点のマーカーの色に変更する。
例1、マーカーを付けたポータルが、レベルアップしてる場合、マーカーの色を変更する。チェック時のポータルレベルより高いかはポータル情報読込時に毎回チェックする。
例2、ポータルが攻撃によりレベルを下げたとき、ポータルレベルは下がっているため指定のポータルレベルのマーカーに状態を変更する。ポータルLv0（ニュートラル）状態の場合確実にマーカーを削除する。
例3、チェック無しレベルに該当する場合、マーカーを削除する。この機能は、ポータルレベル変動に応じてマーカーを変動させる旨のチェックボックスにてON/OFF可能にする。
[17] タップ時のマーカー付与機能は明瞭にする必要がある。ラジオボタン形式で「何もしない」「マーカー付与のみ」「トグルモード」「マーカー削除のみ」に単純化せよ。
[18] 視界に存在するポータルにマーカーを付与する機能が必要だ。チェックが入っているポータルを対象に、クリックを不要としたマーカー付与を行うボタンを設置せよ。デフォルトはトグルモード。選択したモードは記憶しリロードでも消えないようにすること。このナンバーの機能は独立してボタンやリストを作成する必要はない。他の機能の補強メモである。
[19] MACHINA陣営は window.TEAM_MAC である。マーカー処理対象の陣営に追加せよ。
