...
[18] 視界に存在するポータルにマーカーを付与する機能が必要だ。チェックが入っているポータルを対象に、クリックを不要としたマーカー付与を行うボタンを設置せよ。デフォルトはトグルモード。選択したモードは記憶しリロードでも消えないようにすること。
[19] マーカーの一括付与では _̶̱̍_̴̳͉̆̈́M̷͔̤͒Ą̷̍C̴̼̕ͅH̶̹͕̼̾Ḭ̵̇̾̓N̵̺͕͒̀̍Ä̴̞̰́_̴̦̀͆̓_̷̣̈́  という通称MACHINAというNPCユーザ名の占領するポータルは除外せよ。
