...
[15] ▼マーカーをポータルタップで追加、再タップで削除出来るよう、トグル機能が必要。▼マーカーに触っても削除できるようにする。これは機能をON/OFFで切り替えれるようにする。デフォルトはON。削除モードとバッティングするため、トグル機能がONになったら、削除モードは強制的にオフにし、選択出来ないようにする。トグルモードがOFFの時は、通常通りポータルタップで▼マーカーだけ付けれる状態にする。
...
[18] 視界に存在するポータルにマーカーを付与する機能が必要だ。チェックが入っているポータルを対象に、クリックを不要としたマーカー付与を行うボタンを設置せよ。
...
各バージョンごとに、IITC plugin- PortalSlayer-0.9.23.user.js のように連番ファイルを新しく生成すること。これはユーザーのバージョン管理のためである。
