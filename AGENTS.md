...
[15] ▼マーカーをポータルタップで追加、再タップで削除出来るよう、トグル機能が必要。▼マーカーに触っても削除できるようにする。デフォルトはなにもしない。ラジオボタンで▼マーカー付与モードを切り替えられるため動作バッティングを考慮不要。選択したモードは記憶しリロードでも消えないようにすること。
...
[18] 視界に存在するポータルにマーカーを付与する機能が必要だ。チェックが入っているポータルを対象に、クリックを不要としたマーカー付与を行うボタンを設置せよ。選択したモードは記憶しリロードでも消えないようにすること。
