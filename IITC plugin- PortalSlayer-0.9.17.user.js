...
// @version        0.9.17
// @description    [0.9.17] Android最適化: CSS制御による軽量化(激重バグ修正)
// @id             portal-slayer
...
function wrapper(plugin_info) {
  ...
  const DEFAULT_OPTS = {
    ...
    expertDataZoomOffset: 0, // 0=Default, 1-4=Offset, 99=ForceAll
    expertRequestParallel: 5, // Default IITC is 5
    expertRequestDelay: 0     // Default IITC is 0 (ms)
  };
  ...
  S.applyExpertOverrides = function() {
    ...
    if (S.options.expertDataZoomOffset !== 0) {
        ...
        window.getDataZoomForMapZoom = function(mapZoom) {
            // Force All (L1+) usually means max detail (zoom 17-21)
            if (S.options.expertDataZoomOffset === 99) return 21;

            let z = S.originalGetDataZoom(mapZoom);
            z += S.options.expertDataZoomOffset;
    ...
    if (window.mapDataRequest) {
        const parallel = parseInt(S.options.expertRequestParallel || 5, 10);
        const delay = parseInt(S.options.expertRequestDelay || 0, 10);

        // Apply immediately
        window.mapDataRequest.MAX_REQUESTS = parallel;
        window.mapDataRequest.RUN_QUEUE_DELAY = delay;
    }
  };
  ...
  // ============================================================
  // [v0.9.17] Real Portal Filtering Logic (CSS-based Optimization)
  // ============================================================
  ...
  S.updateGamePortalVisibility = function() {
    ...
    for (const guid in window.portals) {
        ...
        // If _icon is undefined, skip it (Leaflet handles clipping)
        if (!p._icon) continue;
        ...
        if (S.shouldShowPortal(level, zoom)) {
             L.DomUtil.removeClass(p._icon, 'plugin-portal-slayer-hidden');
        } else {
             L.DomUtil.addClass(p._icon, 'plugin-portal-slayer-hidden');
        }
    }
  };
  ...
  S.checkPortalVisibility = function(data) {
    ...
    apply();
    // Safety check in case icon wasn't ready immediately
    if (!p._icon) {
        setTimeout(apply, 0);
    }
  };
  ...
  S.openSettings = function() {
    ...
    const html = `
      <div class="portal-slayer-settings">
        <div class="ps-header">
           ...
           <div style="margin-top:8px; border-top:1px solid #444; padding-top:4px;">
             <div style="font-weight:bold; color:#ff5555;">【危険】エキスパート設定 (Banリスクあり):</div>
             <div style="color:#f88; font-size:11px; margin-bottom:4px;">⚠️ サーバー高負荷 / Banリスク。待機時間を設定してください！</div>

             <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
               <label style="flex:1;">データ取得ズーム補正 (God Eye):</label>
               <select id="ps-expert-offset" style="flex:1;">
                 <option value="0">0 (無効/安全)</option>
                 <option value="1">+1</option>
                 <option value="2">+2</option>
                 <option value="3">+3</option>
                 <option value="4">+4</option>
                 <option value="99">全開 (L1+ 強制取得)</option>
               </select>
             </div>

             <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
               <label style="flex:1;">同時リクエスト数 (並列数):</label>
               <input type="number" id="ps-expert-parallel" value="5" min="1" max="10" style="flex:1;">
             </div>

             <div style="display:flex; align-items:center; gap:8px;">
               <label style="flex:1;">リクエスト待機時間 (ms):</label>
               <input type="number" id="ps-expert-delay" value="0" min="0" step="100" style="flex:1;">
             </div>
           </div>
        </div>
        ...
        <table class="ps-level-table">
          <tr><th>Lv</th><th>自動</th><th>色</th><th>最小ズーム</th><th>最大ズーム</th></tr>
    ...
    $('#ps-expert-offset').on('change', function() {
        S.options.expertDataZoomOffset = parseInt(this.value, 10);
        S.saveSettings();
        S.applyExpertOverrides();
    });
    $('#ps-expert-offset').val(S.options.expertDataZoomOffset || 0);

    $('#ps-expert-parallel').on('change', function() {
        S.options.expertRequestParallel = parseInt(this.value, 10);
        S.saveSettings();
        S.applyExpertOverrides();
    });
    $('#ps-expert-parallel').val(S.options.expertRequestParallel || 5);

    $('#ps-expert-delay').on('change', function() {
        S.options.expertRequestDelay = parseInt(this.value, 10);
        S.saveSettings();
        S.applyExpertOverrides();
    });
    $('#ps-expert-delay').val(S.options.expertRequestDelay || 0);
  ...
  S.setupCSS = function() {
    if ($('#portal-slayer-css').length === 0) {
      $('<style>').prop('id', 'portal-slayer-css').prop('type', 'text/css').html(`
        ...
        /* [0.9.17] Optimization: Hidden class */
        .plugin-portal-slayer-hidden {
    ...
    try {
      ...
      const initMap = setInterval(function() {
        if (window.map && window.L) {
          ...
          // [0.9.17] New Hooks for Real Portal Filtering
...
(function() {
  var info = { "script": { "name": "IITC plugin: PortalSlayer", "version": "0.9.17", "description": "[0.9.17] Android最適化: CSS制御による軽量化(激重バグ修正)" } };
