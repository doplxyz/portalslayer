...
// @version        0.9.19
// @description    [0.9.19] 機能追加: 動作プリセット＆L8カウンター
// @id             portal-slayer
        ...
        <div class="ps-header">
           ...
           <div style="margin-top:8px; border-top:1px solid #444; padding-top:4px;">
             ...
             <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
               <label style="flex:1;">動作モード設定 (プリセット):</label>
  ...
  S.updatePortalCounter = function() {
    ...
    if (window.portals) {
      ...
      for (let i = 0; i < total; i++) {
         ...
         if (p && p.options && p.options.data && p.options.data.level === 8) {
             l8++;
         }
      }
    }
    $('#portal-slayer-counter').text(`P: ${l8} / ${total}`);
  };
...
(function() {
  var info = { "script": { "name": "IITC plugin: PortalSlayer", "version": "0.9.19", "description": "[0.9.19] 機能追加: 動作プリセット＆L8カウンター" } };
