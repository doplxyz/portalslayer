...
// @version        0.9.13
// @description    [0.9.13] Expert Mode (God Eye), Request Throttling. Android向け。指定レベル・陣営のポータルをタップ時にマーカー(▼)付与。ポータル名強制表示対応。エリア管理・リスト表示機能追加。ズームレベル表示位置修正・描画距離設定削除。
// @id             portal-slayer
    ...
    const html = `
      <div class="portal-slayer-settings">
        <div class="ps-header">
           ...
           <div style="margin-top:8px; border-top:1px solid #444; padding-top:4px;">
             <div style="font-weight:bold; color:#ff5555;">Expert Data Settings (Risk!):</div>
             <div style="color:#f88; font-size:11px; margin-bottom:4px;">⚠️ High Server Load / Ban Risk. Use 'Request Delay'!</div>

             <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
               <label style="flex:1;">Data Detail Offset:</label>
               <select id="ps-expert-offset" style="flex:1;">
                 <option value="0">0 (Default/Safe)</option>
                 <option value="1">+1</option>
                 <option value="2">+2</option>
                 <option value="3">+3</option>
                 <option value="4">+4</option>
                 <option value="99">Force All (L1+)</option>
               </select>
             </div>

             <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
               <label style="flex:1;">Parallel Requests:</label>
               <input type="number" id="ps-expert-parallel" value="5" min="1" max="10" style="flex:1;">
             </div>

             <div style="display:flex; align-items:center; gap:8px;">
               <label style="flex:1;">Request Delay (ms):</label>
...
(function() {
  var info = { "script": { "name": "IITC plugin: PortalSlayer", "version": "0.9.13", "description": "[0.9.13] Expert Mode (God Eye), Request Throttling. Android向け。指定レベル・陣営のポータルをタップ時にマーカー(▼)付与。ポータル名強制表示対応。エリア管理・リスト表示機能追加。ズームレベル表示位置修正・描画距離設定削除。" } };
